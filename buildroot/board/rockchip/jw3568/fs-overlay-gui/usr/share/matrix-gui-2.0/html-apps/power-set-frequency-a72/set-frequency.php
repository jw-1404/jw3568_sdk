...
<?php 
	...
	$output = shell_exec('cat /sys/devices/system/cpu/cpu4/cpufreq/scaling_available_frequencies');
	$output = trim($output);
	$output = explode (" ",$output);

	$current_governor = shell_exec('cat /sys/devices/system/cpu/cpu4/cpufreq/scaling_governor');
	$current_governor = trim($current_governor);

	$current_freq = shell_exec('cat /sys/devices/system/cpu/cpu4/cpufreq/cpuinfo_cur_freq');
...
<?php
	for($i=0;$i<count($output);$i++)
	{
		...
		echo "<a href = '#' value = 'setclockspeed_a72.sh $output[$i]' class = 'myLink' ><img class = 'app_icon' src= '/apps/images/power-icon.png' ></a>";
