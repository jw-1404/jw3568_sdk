...
function usage()
{
	...
	echo "ubuntu18           -build ubuntu18.04 rootfs"
	echo "ubuntu20           -build ubuntu20.04 rootfs"
	...
	echo "115200/1500000	 -build change the baud rate of the UBOOT serial port to 115200 or 150000"
	echo ""
	echo "Default option is 'allsave'."
}

function build_info(){
	if [ ! -d $TARGET_PRODUCT_DIR ];then
...
function build_kernel(){
	...
	make ARCH=$RK_ARCH modules_install INSTALL_MOD_PATH=$TOP_DIR/buildroot/output/$RK_CFG_BUILDROOT/target/
...
function build_ubuntu18(){
        #check_config RK_CFG_UBUNTU || return 0

        echo "==========Start building ubuntu-18.04=========="
        #echo "TARGET_UBUNTU_CONFIG=$RK_CFG_UBUNTU"
        echo "========================================="

	cd ubuntu-18.04
        /usr/bin/time -f "you take %E to build ubuntu-18.04" \
		sudo ./mk-ubuntu.sh
	/usr/bin/time -f "you take %E to build ubuntu-18.04 image" \
		sudo ./mk-image.sh
	cd -
...
function build_ubuntu20(){
        #check_config RK_CFG_UBUNTU || return 0

        echo "==========Start building ubuntu-20.04=========="
        #echo "TARGET_UBUNTU_CONFIG=$RK_CFG_UBUNTU"
        echo "========================================="

        cd ubuntu-20.04
        /usr/bin/time -f "you take %E to build ubuntu-20.04" \
                sudo ./mk-ubuntu.sh
        cd -
...
function build_uboot_uart_baudrate() {
	echo "Change the baud rate of UBOOT serial port to ${1}"
	## modify uboot
	sed -i "s/CONFIG_BAUDRATE=[0-9]*/CONFIG_BAUDRATE=$1/g" $TOP_DIR/u-boot/configs/OK3568-C_defconfig

	## modify kernel
	sed -i "s/rockchip,baudrate.*=.*<[0-9]*>/rockchip,baudrate = <${1}>/g" $TOP_DIR/kernel/arch/arm64/boot/dts/rockchip/OK3568-C-common.dtsi

	## modify sdk
	buildroot_baudrate=`cat ${TOP_DIR}/buildroot/configs/OK3568_defconfig | grep BR2_TARGET_GENERIC_GETTY_BAUDRATE`
	sed -i "s/.*$buildroot_baudrate.*/BR2_TARGET_GENERIC_GETTY_BAUDRATE_$1=y/g" $TOP_DIR/buildroot/configs/OK3568_defconfig

	RKBIN=$TOP_DIR/rkbin
	CHIP_PATTERN='^CONFIG_ROCKCHIP_[R,P][X,V,K][0-9ESX]{1,5}'
	RKCHIP=`egrep -o ${CHIP_PATTERN} ${TOP_DIR}/u-boot/configs/OK3568-C_defconfig`
	RKCHIP=${RKCHIP##*_}

	INI_LOADER=${RKBIN}/RKBOOT/${RKCHIP}MINIALL.ini
	NAME=`sed -n "/CONFIG_LOADER_INI=/s/CONFIG_LOADER_INI=//p" ${TOP_DIR}/u-boot/configs/OK3568-C_defconfig |tr -d '\r' | tr -d '"'`
	if [ ! -z "${NAME}" ]; then
		INI_LOADER=${RKBIN}/RKBOOT/${NAME}
	fi
	INI=`cat $INI_LOADER | grep ddr | head -n 1 | awk -F '=' '{print $2}'`

	sed -i "s/uart baudrate=[0-9]*/uart baudrate=$1/g" ${RKBIN}/tools/ddrbin_param.txt
	${RKBIN}/tools/ddrbin_tool ${RKBIN}/tools/ddrbin_param.txt ${RKBIN}/${INI}
}
...
for option in ${OPTIONS}; do
	...
	case $option in
		...
		*.mk)
		...
		rootfs|buildroot|ubuntu18|ubuntu20|debian|distro|yocto) build_rootfs $option ;;
		pcba) build_pcba ;;
		...
		115200|1500000) build_uboot_uart_baudrate $1 ;;
		*) usage ;;
	esac
done
