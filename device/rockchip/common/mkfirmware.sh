...
then
	...
	then
		...
		if [ "$RK_OEM_FS_TYPE" = "ubi" ]; then
			...
			OEM_SIZE=$(du -m $OEM_DIR | grep -o "^[0-9]*" | tail  -n 1)
			OEM_SIZE=$((((($OEM_SIZE) + (4)) & ~(4))))
			echo "$MKIMAGE $OEM_DIR $ROCKDEV/oem.img $RK_OEM_FS_TYPE $OEM_SIZE"  >> $OEM_FAKEROOT_SCRIPT
		fi
...
then
	...
	then
		...
		if [ "$RK_USERDATA_FS_TYPE" = "ubi" ]; then
			...
			USERDATA_SIZE=$(du -m $USER_DATA_DIR | grep -o "^[0-9]*" | tail  -n 1)
			USERDATA_SIZE=$((((($USERDATA_SIZE) + 32) & ~(4))))
			echo "$MKIMAGE $USER_DATA_DIR $ROCKDEV/userdata.img $RK_USERDATA_FS_TYPE $USERDATA_SIZE"  >> $USERDATA_FAKEROOT_SCRIPT
		fi
